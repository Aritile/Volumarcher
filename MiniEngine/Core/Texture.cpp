...
void Texture::Create2DArray(size_t RowPitchBytes, size_t Width, size_t Height, size_t Depth, DXGI_FORMAT Format,
                            const void* InitialData, unsigned int mipLevels, D3D12_RESOURCE_FLAGS flags,
                            size_t slicePitchBytes)
{
	Destroy();
	...
	texResource.pData = InitialData;
	texResource.RowPitch = RowPitchBytes;
	if (slicePitchBytes == -1)
	{
		texResource.SlicePitch = RowPitchBytes * Height;
	}
	else
	{
		texResource.SlicePitch = slicePitchBytes;
	}
...
void Texture::CreateTGAFromMemory(const void* _filePtr, size_t, bool sRGB)
{
	...
	*filePtr++;
	...
	Create2D(4 * imageWidth, imageWidth, imageHeight,
	         sRGB ? DXGI_FORMAT_R8G8B8A8_UNORM_SRGB : DXGI_FORMAT_R8G8B8A8_UNORM, formattedData);
...
bool Texture::CreateDDSFromMemory(const void* filePtr, size_t fileSize, bool sRGB)
{
	...
	HRESULT hr = CreateDDSTextureFromMemory(Graphics::g_Device,
	                                        (const uint8_t*)filePtr, fileSize, 0, sRGB, &m_pResource,
